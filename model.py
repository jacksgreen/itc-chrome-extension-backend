...
def predict(data, dct, scrape):
    ...
    data = data[(data["price"] > 0.6 * dct["price"]) & (data["price"] < 1.4 * dct["price"]) & (data["title"] != " ")].sort_values(by="ecoscore", ascending=False)[:3].set_index(pd.Index(["firstSuggestion", "secondSuggestion", "thirdSuggestion"]))
